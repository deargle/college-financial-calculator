<template>
  <div id="app" class='container pt-4' v-cloak>

    <h1>College Financial Calculator</h1>
    <div>
        <p class='meta'>2018-08-09 by <a href='https://daveeargle.com'>Dave Eargle</a></p>

        <p>Use this calculator to determine how much you need to invest using
        one of three strategies (monthly, annually, or one-time up-front) in
        order to have sufficient funds for future expenses. Once the future
        expenses start, they continue regularly, and inflate annually, until the
        specified end time. This calculator is just for fun.</p>

        <p>Associated blog post <a href='https://daveeargle.com/2018/08/09/College-Financial-Calculator/'>here</a>.</p>
        ...
        <div>
            <form id='per-semester'>
                ...
                <div class='form-row align-items-center'>
                    <div class='col-auto'>
                        <input @click='do_all_goal_seeks' class='btn btn-primary' type='button' id='do-goal-seek' value='Optimize Investment Strategies'>
                    </div>
                    <div class='col-sm-3'>
            ...
            <template>
                ...
                <table class='table table-sm'>
                    ...
                    <tbody>
                        <tr :key="total.method" v-for='(total) in [
                                {
                                    method: "monthly",
            ...
            <template v-for='cashflow in cashflows_meta_filtered'>
                <a
                  :key="cashflow.id"
                  class='btn'
                  ...
                  role='button'
                  aria-expanded='false'
                  :aria-controls='cashflow.href'>{{ cashflow.method }}</a>
            </template>
            <div class='accordion' id='cashflows-container'>
                <template v-for='cashflow in cashflows_meta_filtered'>
                    <div :key="cashflow.id" :id='cashflow.id' class='collapse' data-parent='#cashflows-container'>
                        <div class='card card-body'>
                            ...
                            <table class='table table-sm'>
                                ...
                                <tbody>
                                    <tr :key="index" v-for='(flow, index) in cashflow.flows'>
                                        <td>{{ flow.period }}</td>
...
function drawChart() {
    ...
    let svg = svg_holder.append('svg').attr('width', svg_width).attr('height', svg_height);
    let g = svg.append('g')
              .attr('id', 'svg-inner')
...
function updateChart(cashflows_hash) {
    let cashflows_list = cashflows_hash.map(function (d) { return d.value });
    let cashflows_name = cashflows_hash.map(function (d) { return d.name });
    ...
    let paths = pathContainers.selectAll('path')
...
export default {
  ...
  computed: {
      cashflows_meta_filtered() {
        return this.cashflow_meta.filter( (meta) => { return meta.flows.length > 0 })
      },
      ...
      payment_periods: function() {
          ...
          for(var n = 0; n < this.months_in_school; n++) {
              let _pp = this.months_until_start + n;
  ...
  methods: {
      sum_investment_total(investments) {
          const reducer = (accumulator, currentValue) => accumulator + currentValue;
          return investments.reduce(reducer);
      },
      generate_cashflows(investments) {
          var data = [];
          var fv = 0;
          for (var n = 0; n < this.nper; n++) {

              let money_in = investments[n];

              let money_out = this.payment_amounts[n];

              let net_cashflow = money_in - money_out;
      ...
      final_fv_for_monthly_investment(monthly_investment) {
          this.monthly_investment = monthly_investment;
          return this.monthly_last_fv;
      },
      final_fv_for_annual_investment(annual_investment) {
          this.annual_investment = annual_investment;
          return this.annual_investment_last_fv;
      },
      final_fv_for_upfront_investment(one_time_investment) {
          this.one_time_investment = one_time_investment;
          return this.up_front_last_fv;
      },
      do_all_goal_seeks() {
        this.do_goal_seek(this.final_fv_for_monthly_investment)
        this.do_goal_seek(this.final_fv_for_annual_investment)
        this.do_goal_seek(this.final_fv_for_upfront_investment)
      },
      do_goal_seek(_func) {
          let tol_func = (x) => {
            return x > 0 && x < 200
